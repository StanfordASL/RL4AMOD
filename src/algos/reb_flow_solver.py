...
def solveRebFlow(env, res_path, desiredAcc, CPLEXPATH, directory):
    ...
    err_file = "error.log"
    try:
        with open(out_file, "w") as output_f, open(err_file, "w") as error_f:
            subprocess.check_call(
                [CPLEXPATH + "oplrun", modfile, datafile], stdout=output_f, env=my_env
            )
    except subprocess.CalledProcessError as e:
        print(f"Command '{e.cmd}' returned non-zero exit status {e.returncode}.")
        with open(err_file, "r") as error_f:
            error_message = error_f.read()
            print("Error message:\n", error_message)
        with open(out_file, "r") as output_f:
            output_message = output_f.read()
